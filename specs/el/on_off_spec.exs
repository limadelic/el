defmodule El.Features.OnOffSpec do
  ...
  describe "El.exit/1" do
    setup do
      ...
      stub(El.MockSessionDeletion, :delete_session_messages, fn _, _ -> :ok end)
      :ok
    end

    test "looks up session in registry" do
      expect(El.MockRegistry, :lookup, fn El.Registry, :dude -> [] end)
      El.Lifecycle.exit(:dude, :normal, registry: El.MockRegistry, supervisor: El.MockSupervisor, app: El.MockSessionDeletion, monitor: El.MockMonitor, session_meta: El.MockSessionMeta)
    end
