defmodule El.Session.Handlers.Ask.Spec do
  use ExUnit.Case
  import Mox
  ...
  describe "model plumbing end-to-end" do
    test "model and session_id flow from ask_work through spawn_ask_task to complete_ask cast" do
      ...
      El.Session.Handlers.Ask.spawn_ask(state, ask_info, [], server_pid)
  ...
  describe "finalize_ask/2" do
    ...
    test "calls store with model in metadata when model is provided" do
      ...
      El.Session.Handlers.Ask.finalize_ask(state, %{from: from, ref: ref, message: "question", response: "answer", model: "claude-3"})
    end
    ...
    test "calls store with nil when model is nil" do
      ...
      El.Session.Handlers.Ask.finalize_ask(state, %{from: from, ref: ref, message: "question", response: "answer", model: nil})
    end
    ...
    test "replies to caller with response" do
      ...
      El.Session.Handlers.Ask.finalize_ask(state, %{from: from, ref: make_ref(), message: "test", response: "the answer", model: "claude-3"})
    ...
    test "deletes pending entry from DETS on completion" do
      ...
      returned_state = El.Session.Handlers.Ask.finalize_ask(state, %{from: from, ref: pending_ref, message: "question", response: "answer", model: "claude-3"})
  ...
  describe "prepare_ask/3" do
    ...
    test "stores pending entry with empty response on prepare_ask" do
      ...
      {_ref, new_state} = El.Session.Handlers.Ask.prepare_ask(state, from, "test question")
    ...
    test "does not store pending entry when ask has routes" do
      ...
      {_ref, new_state} = El.Session.Handlers.Ask.prepare_ask(state, from, "@target> routed question")
  ...
  describe "reset_session/1" do
    ...
    test "reset_session leaves session_id as nil", %{state: state} do
      new_state = El.Session.Handlers.Ask.reset_session(state)
    ...
    test "reset_session strips :session_id from claude_opts", %{state: state} do
      ...
      new_state = El.Session.Handlers.Ask.reset_session(state_with_session_id)
    ...
    test "deletes DETS messages via store_module.delete_session_messages", %{state: state} do
      ...
      El.Session.Handlers.Ask.reset_session(state)
