defmodule El.Features.El2ElSpec do
  ...
  describe "El.tell/2" do
    test "routes message to target session" do
      ...
      assert El.tell(:dude, "@donnie> test message", session_api: El.MockSessionApi) == :ok
    end
    ...
    test "passes exact message to session" do
      ...
      assert El.tell(:dude, expected_msg, session_api: El.MockSessionApi) == :ok
    end

    test "returns ok" do
      stub(El.MockSessionApi, :tell, fn _, _ -> :ok end)
      result = El.tell(:dude, "message", session_api: El.MockSessionApi)
      assert result == :ok
    end
  end
  ...
  describe "El.ask/2" do
    test "sends ask to session" do
      ...
      assert El.ask(:dude, "1 + 1", session_api: El.MockSessionApi) == "2"
    end

    test "returns response from session" do
      stub(El.MockSessionApi, :ask, fn _, _ -> "response text" end)
      response = El.ask(:dude, "question", session_api: El.MockSessionApi)
    ...
    test "routes to target session in message" do
      ...
      result = El.ask(:dude, expected_msg, session_api: El.MockSessionApi)
  ...
  describe "El.log/1" do
    test "fetches log from session" do
      ...
      log = El.log(:dude, nil, session_api: El.MockSessionApi)
  ...
  describe "El.log/2" do
    test "delegates to session with name and count" do
      ...
      assert El.log(:dude, 5, session_api: El.MockSessionApi) == :ok
    end
  end
end
